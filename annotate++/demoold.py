...
class Predictor(object):
    ...
    def visualize(self, img, dets, meta, class_names, score_thres, wait=0):
        time1 = time.time()
...
def main():
    ...
    print("_____________________________________________",predictor)
    ...
    if args.demo == 'image':
        ...
        for image_name in files:
            meta, res = predictor.inference(image_name, self.scores)
            result_image = predictor.visualize(res, meta, cfg.class_names, 0.35)
            # sc = scores_con(image_name, dets, class_names, score_thresh)
            if args.save_result:
